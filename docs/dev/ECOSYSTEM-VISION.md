...
│  │              NanoClaw (에이전트 런타임)           │   │
│  │                                                  │   │
│  │  컨테이너 격리 │ MCP │ 멀티채널 │ BYOK           │   │
│  └──────────────────────┬───────────────────────────┘   │
│                         │                               │
│  ┌──────────────────────┴───────────────────────────┐   │
│  │              맥미니 (하드웨어)                      │   │
│  │  Docker │ Ollama (로컬 LLM) │ 파일시스템           │   │
│  └──────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────┘
```
...
| 계층 | 역할 | 비유 |
|------|------|------|
| **ClawTalk** | 에이전트와 사람의 소통 | 입과 귀 |
| **ClawNote** | 지식 저장, 연결, 회상 | 기억과 뇌 |
| **NanoClaw** | 에이전트 실행, 도구 사용 | 손과 발 |
| **ClawOS** | 설치, 관리, 업데이트 | 신경계 |
...
### 1. 4개 제품이 아니라, 1개 생태계의 4개 계층이다
ClawTalk(소통) + ClawNote(기억) + NanoClaw(행동) + ClawOS(관리)는 독립 제품이 아니라 **하나의 디지털 자아를 구성하는 계층**. 하지만 기술적으로는 느슨하게 결합되어야 한다.

### 2. "파일은 제자리, 인덱스로 연결" — Federated Memory
기존 파일/구조를 옮기지 않고, 통합 인덱스(Knowledge Graph + 벡터)로 연결. 마크다운과 SQLite라는 표준 형식을 고수하여 락인 방지.

### 3. ClawTalk 먼저, 나머지는 순차적으로
지금은 ClawTalk에 집중. ClawNote는 ClawTalk이 안정된 후. ClawOS와 통합 지식 그래프는 더 나중에. **한 번에 하나씩, 각각 PMF를 확인하면서 진행.**
